-- Rythme du montage et images de fin de plan (métadonnées 'pace' et
-- 'end_frames'). Rythme rapide : deux fois plus de plans pour une même
-- durée, donc coût doublé. Image de fin : un crédit de plus par plan.
-- Aligné avec costOf (src/lib/generation.ts).
...
-- Image de fin de plan : deuxième image, générée à partir de la première.
alter table public.generation_shots
  add column end_image_prompt text,
  add column end_image_prediction_id text,
  add column end_image_path text;

-- Un deuxième essai pour l'image de fin : le filtre de contenu du modèle
-- d'image refuse parfois une requête sans raison stable.
alter table public.generation_shots
  add column end_image_attempts integer not null default 0;
