-- Remplacement en trois étapes par plan (image clé, vidéo, contrôle par
-- Claude, plan refait une fois s'il est raté) : 2,5 crédits la seconde au
-- lieu de 2, plus 3 crédits de fiche personnage. Aligné avec swapCredits
-- (generation.ts).
...
begin
  ...
  v_cost := ceil(p_duration_seconds * 2.5) + 3;
